...
let player = new mainPlayerCharacter("병주", 100, 10, 200, 0.5, 1000, 20);
//                                       atk def hp stealSuccessPer level(난이도정도로 생각) money
let muheon = new createCharacters("무헌", 210, 10, 100, 0, 10, 1000);
let professor = new createCharacters("교수", 20, 10, 100, 5, 20, 1000);
let boss_lee = new createCharacters("학과장", 20, 10, 100, 10, 100, 10000);
let hyunwook = new createCharacters("현욱", 20, 10, 100, 10, 10, 1000);
let ahyeon = new createCharacters("아현", 20, 10, 100, 10, 5, 1000);
let jisup = new createCharacters("지섭", 20, 10, 100, 10, 1, 1000);
let megacoffee = new createCharacters("메가커피", 20, 10, 100, 10, 30, 1000);
...
function monsterHpBarChange(){
    ...
    monsterHpChange.style.width = `${monsterHpBarLength}px`;
    if(monsterHpChange.style.width === `400px`){
        return;
    }
}
...
function monsterAppear(){
    ...
    monsterHpSetting.innerHTML = `HP : ${monster[0].hp}`;
    // monsterHpBar();
    imageSelect();
...
function monsterAttack(){
    ...
    playerHpSetting.innerHTML = `HP : ${player.hp}`;
    if(player.hp <= 0){
        setTimeout(() => {
